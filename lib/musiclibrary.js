...
exports.get = function(path) {
  if(libraries[path]) {
    ...
    var numStations = 1 + Math.ceil(Math.random(new Date().getTime()) * 3);
    for(var s=0; s < numStations; s++) {
      ...
      var numGenres = 1 + Math.ceil(Math.random(new Date().getTime()) * 5);
      var usedGenres = [];
      ...
      for(var g=0; g<numGenres; g++) {
        ...
        var genre = {
          ...
          songs: getSongs(genreName, getColor(genreName), 300)
        };
